...
export async function listAllNamespacedCloudProfiles ({ user, diff = false }) {
  ...
  const filteredItems = allItems.filter(item => allowedNamespaces.includes(item.metadata.namespace))
  const simplifiedItems = filteredItems.map(simplifyCloudProfile)
